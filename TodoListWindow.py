...
def todoList(email):
    ...
    def add(email):
        ...
        cursor.execute("SELECT MAX(TaskNo) FROM SCHEMA_NAME.TaskTodoList WHERE EmailId = :email", email=email)
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO SCHEMA_NAME.TaskTodoList (EmailId, TaskNo, TaskName, TaskDate, TaskTime) 
                VALUES (:1, :2, :3, :4, :5)
    ...
    def load_tasks():
        for i in tree.get_children():
            tree.delete(i)
        cursor.execute("SELECT TaskNo, TaskName, TaskDate, TaskTime FROM SCHEMA_NAME.TaskTodoList WHERE emailid=:u ORDER BY TaskNo ASC", u=email)
    ...
    def search_tasks():
        ...
        for i in tree.get_children():
            tree.delete(i)
        cursor.execute("SELECT TaskNo, TaskName, TaskDate, TaskTime FROM SCHEMA_NAME.TaskTodoList WHERE emailid=:u AND TaskName LIKE :1", {'u': email, '1': f'%{search_term}%'})
        records = cursor.fetchall()
    ...
    def delete_task():
        ...
        cursor.execute("DELETE FROM SCHEMA_NAME.TaskTodoList WHERE TaskNo = :1 AND emailid = :2", (task_no, email))
        con.commit()
        
        # Re-number the remaining tasks
        cursor.execute("SELECT TaskNo FROM SCHEMA_NAME.TaskTodoList WHERE emailid = :1 ORDER BY TaskNo ASC", (email,))
        records = cursor.fetchall()
        ...
        for new_task_no, (old_task_no,) in enumerate(records, start=1):
            if new_task_no != old_task_no:
                cursor.execute("UPDATE SCHEMA_NAME.TaskTodoList SET TaskNo = :1 WHERE TaskNo = :2 AND emailid = :3", (new_task_no, old_task_no, email))
    ...
    def update_task():
        ...
        def save_changes():
            ...
            cursor.execute("""
                UPDATE SCHEMA_NAME.TaskTodoList 
